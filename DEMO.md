...
| Toast: "Your browser blocked the Gmail window" | The draft opens ~12s after your click, outside the window browsers allow pop-ups in | Click the blue **Open draft** button in the list — that counts as a click, so it is never blocked. To stop it happening at all, allow pop-ups for the site: click the blocked-pop-up icon in the address bar → Always allow. |
